...
void *do_work(void *arg) {
    ...
    while (1) {
        ...
        while (p->next != NULL) {
            ...
            naddr.sin_port = htons(CTLPORT);
            naddr.sin_addr = p->next->client_addr.sin_addr;
            connect(askfd, (struct sockaddr *)&naddr, sizeof(struct sockaddr));
            fprintf(fp, "%s:%d\n", inet_ntoa(naddr.sin_addr), ntohs(naddr.sin_addr.s_addr));
            ev.events = EPOLLOUT;
            ev.data.fd = askfd;
        ...
        int reval = epoll_wait(epollfd, events, MAXCLIENT, 1000);
        if(reval < 0) {
